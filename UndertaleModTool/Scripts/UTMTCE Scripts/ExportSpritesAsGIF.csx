...
void ExtractSprite(UndertaleSprite sprite, string folder, TextureWorker worker)
{
    ...
    for (int picCount = 0; picCount < sprite.Textures.Count; picCount++)
    {
        if (sprite.Textures[picCount]?.Texture != null)
        {
            ...
            if (sprite.IsSpecialType && Data.GeneralInfo?.Major >= 2) {
                if (sprite.GMS2PlaybackSpeed == 0f) {
                    ...
                    image.AnimationDelay = Math.Max((int)(Math.Round(100f / (sprite.GMS2PlaybackSpeed * Data.GeneralInfo.GMS2FPS))), 1);
                } else {
                    image.AnimationDelay = Math.Max((int)(Math.Round(100 / sprite.GMS2PlaybackSpeed)), 1);
                }
            } else {
