    ...
    public static partial class Compiler
    {
        public static class AssemblyWriter
        {
            ...
            public class Context
            {
                ...
                public Context(Patch @break, Patch @continue)
                {
                    Break = @break;
                    Continue = @continue;
                }
            ...
            public class OtherContext : Context
            {
                ...
                public OtherContext(Patch @break, Patch @continue, DataType typeToPop) : base(@break, @continue)
                {
                    Kind = ContextKind.Switch;
                    TypeToPop = typeToPop;
                }

                public OtherContext(Patch @break, Patch @continue) : base(@break, @continue)
                {
            ...
            private static Patch HelpUseBreak(ref Stack<Context> s)
            {
                Context c = s.Pop();
            ...
            private static Patch HelpUseContinue(ref Stack<Context> s)
            {
                Context c = s.Pop();
            ...
            private static void AssembleStatement(CodeWriter cw, Parser.Statement s, int remaining = -1)
            {
                switch (s.Kind)
                {
                    ...
                    case Parser.Statement.StatementKind.ForLoop:
                        {
                            ...
                            cw.loopContexts.Push(new Context(endLoopPatch, continuePatch));
                    ...
                    case Parser.Statement.StatementKind.WhileLoop:
                        {
                            ...
                            endLoopPatch.Add(cw.Emit(Opcode.Bf));
                            cw.loopContexts.Push(new Context(endLoopPatch, conditionPatch));
                            cw.contexts.Push(cw.loopContexts.Peek());
                            AssembleStatement(cw, s.Children[1]); // body
                            cw.loopContexts.Pop();
                            cw.contexts.Pop();
                    ...
                    case Parser.Statement.StatementKind.RepeatLoop:
                        {
                            ...
                            cw.loopContexts.Push(new Context(endPatch, repeatPatch));
                            cw.contexts.Push(cw.loopContexts.Peek());
                    ...
                    case Parser.Statement.StatementKind.Switch:
                        {
                            ...
                            Context enclosingContext = default(Context);
                            if (isEnclosingLoop)
                            {
                            ...
                            cw.contexts.Push(cw.otherContexts.Peek());
                    ...
                    case Parser.Statement.StatementKind.Continue:
                        ...
                        {
                            HelpUseContinue(ref cw.contexts).Add(cw.Emit(Opcode.B));
                        }
                        break;
                    case Parser.Statement.StatementKind.Break:
                        if (cw.loopContexts.Count == 0 && cw.otherContexts.Count == 0)
                        {
                            ...
                            HelpUseBreak(ref cw.contexts).Add(cw.Emit(Opcode.B));
                        break;
            ...
            private static void AssembleFunctionCall(CodeWriter cw, Parser.Statement fc, bool isVariableCall = false, bool isSelf = false)
            {
                ...
                if (isVariableCall)
                {
                    /*
                        self:
                        push.v [variables]
                        call.i @@This@@(argc=0)    ([from])
                        push.v builtin.player_hurt
                        callv.v [argcount]

                        anything else:
                        push.v [from]
                        push.v [variables]
                        dup.v argcount 8 ;;; this is a weird GMS2.3+ swap instruction
                        dup.v 0
                        push.v stacktop.has_value
                        callv.v argcount
                    */
                    byte argCount = (byte)(fc.Children.Count - 1);

                    if (!isSelf) {
                        var dupSwap = cw.Emit(Opcode.Dup, DataType.Variable);
                        dupSwap.Extra = argCount;
                        dupSwap.ComparisonKind = (ComparisonType)((byte)8 | 0x80);
                        cw.Emit(Opcode.Dup, DataType.Variable).Extra = 0;
                    }
            ...
            private static void AssembleVariablePush(CodeWriter cw, Parser.Statement e, bool duplicate = false, bool useLongDupForArray = false, bool useNoSpecificType = false, InstanceType forceInstType = InstanceType.Undefined)
            {
                AssembleVariablePush(cw, e, out _, out _, duplicate, useLongDupForArray, useNoSpecificType, forceInstType);
            }

            // Workaround for out parameters #2
            private static void AssembleVariablePush(CodeWriter cw, Parser.Statement e, out bool isSingle, bool duplicate = false, bool useLongDupForArray = false, bool useNoSpecificType = false, InstanceType forceInstType = InstanceType.Undefined)
            {
                AssembleVariablePush(cw, e, out isSingle, out _, duplicate, useLongDupForArray, useNoSpecificType, forceInstType);
            }

            private static void AssembleVariablePush(CodeWriter cw, Parser.Statement e, out bool isSingle, out bool isArray, bool duplicate = false, bool useLongDupForArray = false, bool useNoSpecificType = false, InstanceType forceInstType = InstanceType.Undefined)
            {
                isSingle = false;
                isArray = false;
                        ...
                        switch (id)
                        {
                            ...
                            default:
                                cw.varPatches.Add(new VariablePatch()
                                {
                                    Target = cw.EmitRef(Opcode.Push, DataType.Variable),
                                    Name = name,
                                    InstType = (forceInstType != InstanceType.Undefined) ? forceInstType : (InstanceType)id,
                                    VarType = VariableType.Normal
                                });
                                break;
                        }
                    ...
                    else
                    {
                        ...
                        for (int next = 1; next < e.Children.Count; next++)
                        {
                            if (e.Children[next].Children.Count != 0)
                            {
                                ...
                                {
                                    // Function call
                                    AssembleFunctionCall(cw, e.Children[next], true, isSelf);
                                    isSelf = false;
                                    cw.typeStack.Push(DataType.Variable);
                                    continue;
                                }
                ...
                else if (e.Kind == Parser.Statement.StatementKind.ExprSingleVariable)
                {
                    ...
                    AssembleVariablePush(cw, fix, out isSingle, out isArray, duplicate, useLongDupForArray, useNoSpecificType, forceInstType);
                }
                else
                {
